...
白文字対策として、ページ全体に color: #111111 !important; をかけ、ボタンやリンクも黒で固定しています。画像も display, opacity, visibility を明示してあるので、前より表示されやすい完全版です。

もしまだ画像が出ない場合は、原因は CSS ではなく画像URLかレイアウト側なので、そのときは使っている layout: page 側に合わせてさらに調整します。
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1"> <!-- これが必要 -->
</head>
...
原因は、このページでは layout: page ではなく layout: main を使う前提で、しかもレイアウト側がかなりシンプルなので、こちらもそれに合わせて余計な構造を減らした方が安全です。<head> はレイアウト側に置くべきで、ページ本文には入れない方が安定します。

下の完全版は layout: main 用に整理し直したものです。文字は黒固定、画像は通常表示、英語と日本語はボタン切替のみで表示されます。
